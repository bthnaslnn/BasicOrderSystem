...
namespace WebAPI.Controllers
{
    ...
    public class StoragesController : BaseApiController
    {
        ...
        ///It brings the details according to its id.
        ///</summary>
        ///<remarks>IsExistStorages</remarks>
        ///<return>IsExistStorages</return>
        ///<response code="200"></response>  
        [Produces("application/json", "text/plain")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(bool))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(string))]
        [HttpGet("isexiststorage")]
        public async Task<IActionResult> IsExistStorageByUserIdAndProductId(int userid, int productid)
        {
            var result = await Mediator.Send(new IsExistStorageByUserIdAndProductIdQuery { UserId = userid, ProductId=productid});
            if (result.Success)
            {
                return Ok(result.Data);
            }
